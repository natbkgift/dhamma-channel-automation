...
class PersonalizationAgent(
    BaseAgent[PersonalizationInput, PersonalizationOutput]
):
        ...
        for interest in interests:
            ...
            for entry in entries:
                ...
                if trend_lookup.get(interest):
                    reason_parts.append(
                        f"เทรนด์คะแนน {trend_lookup[interest]:.0f}"
                    )
                if avg_watch is not None and avg_watch >= 80:
                    reason_parts.append(
                        f"Retention เฉลี่ย {avg_watch:.0f}%"
                    )
    ...
    def _build_topic_candidates(
        ...
        for interest in request.profile.interest:
            ...
            for entry in entries:
                ...
                if trend_lookup.get(interest):
                    reason_parts.append(
                        f"เทรนด์ {trend_lookup[interest]:.0f} คะแนน"
                    )
    ...
    def _apply_boosts(
        ...
        if trend_score:
            confidence += min(10.0, trend_score * 0.12)
        if avg_watch is not None:
            confidence += max(-5.0, (avg_watch - 65.0) * 0.15)
        if engagement.total >= 10:
            confidence += 4.0
        elif engagement.total >= 5:
            confidence += 2.0
    ...
    def _average_watch_pct(
        self, view_history: Iterable
    ) -> Optional[float]:
        watched_values: list[float] = []
        for item in view_history:
            pct = getattr(item, "watched_pct", None)
            if pct is None:
                continue
            watched_values.append(float(pct))
    ...
    def _trend_lookup(self, trend: Iterable[TrendInterest]) -> dict[str, float]:
        ...
        for item in trend:
            lookup[item.topic] = float(item.score)
        return lookup
    ...
    def _select_top_candidates(
        ...
        pool_index = 0
        while len(selected) < config.recommend_top_n and any(pools):
    ...
    def _build_action_plan(
        ...
        for item in recommendations:
            if item.type == "video" and item.title:
                plan.append(
                    f"ส่ง push notification โปรโมทวิดีโอ '{item.title}'"
                )
