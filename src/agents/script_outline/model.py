...
class RetentionGoals(BaseModel):
    ...
    mid_segment_break_every_sec: int = Field(description="ช่วงเวลาการแทรก retention pattern", gt=0)
...
class OutlineSection(BaseModel):
    ...
    goal: Optional[str] = Field(None, description="เป้าหมายของส่วนนี้")
    est_seconds: int = Field(description="เวลาประมาณ (วินาที)", gt=0)
    retention_tags: list[str] = Field(description="แท็ก retention pattern")
    
    # Optional fields for specific section types
    hook_pattern: Optional[str] = Field(None, description="รูปแบบ hook")
    content_draft: Optional[str] = Field(None, description="ร่างเนื้อหา")
    key_points: Optional[list[str]] = Field(None, description="ประเด็นสำคัญ")
    analogy_type: Optional[str] = Field(None, description="ประเภทของการเปรียบเทียบ")
    beat_points: Optional[list[str]] = Field(None, description="จุดสำคัญในการเล่าเรื่อง")
    sub_segments: Optional[list[CoreTeachingSubSegment]] = Field(None, description="ส่วนย่อยของการสอน")
    steps: Optional[list[str]] = Field(None, description="ขั้นตอนการปฏิบัติ")
    question: Optional[str] = Field(None, description="คำถามสำหรับ reflection")
    cta_phrase: Optional[str] = Field(None, description="ข้อความ call to action")
    closing_line: Optional[str] = Field(None, description="ประโยคปิด")
    
    @field_validator("section")
    @classmethod
    def validate_section_name(cls, v):
        valid_sections = [
            "Hook", "Problem Amplify", "Transition Prompt", "Story / Analogy",
            "Core Teaching", "Practice / Application", "Reflection Question",
            "Soft CTA", "Calm Closing"
        ]
...
class ConceptCoverage(BaseModel):
    ...
    suggest_add_in_section: Optional[str] = Field(None, description="แนะนำส่วนที่ควรเพิ่ม")
...
class MetaInfo(BaseModel):
    ...
    interrupt_spacing_ok: bool = Field(description="การจัดช่วงเวลา pattern interrupt เหมาะสม")
...
class ScriptOutlineInput(BaseModel):
    ...
    missing_concepts: list[str] = Field(default_factory=list, description="แนวคิดที่ขาดหายไป")
...
class ErrorResponse(BaseModel):
    ...
    error: dict[str, Union[str, Any]] = Field(description="ข้อมูลข้อผิดพลาด")
